...
namespace AatingApp.API.Data {
    public class DatingRepository : IDatingRepository {
        ...
        public async Task<Photo> GetMainPhotoForUser(int usrerId)
        {   
            return await  _context.Photos.Where( u => u.UserId == usrerId).FirstOrDefaultAsync( p=> p.IsMain);
        }
        ...
        public async Task<IEnumerable<Message>> GetMessageThread(int userId, int recipentId)
        {
             var messages = await _context.Messages
                        ...
                        .Where(m => m.RecipientId == userId 
                                && m.RecipientDeleted== false && m.SenderId == recipentId 
                                || m.RecipientId == recipentId && m.SenderDeleted == false 
                                 && m.SenderId == userId)
